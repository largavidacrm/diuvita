#!/usr/bin/env python3
"""Discover official contact/location pages for visible Vitalarga clinics.

Default mode is dry-run. Apply mode stores source_records only; it never edits
clinic profiles, creates public pages, resolves review_queue items or publishes.
"""
...
DISCOVERER_NAME = "vitalarga-location-source-discoverer"
DISCOVERER_VERSION = "2026-08-30"
LOCATION_SOURCE_TYPE = "official_location_page"
MAX_LINKS_PER_CLINIC = 3

LOCATION_TERMS = (
    ("donde estamos", 32),
    ("donde encontrarnos", 30),
    ("ubicacion", 30),
    ("ubicaciones", 30),
    ("localizacion", 28),
    ("localizaciones", 28),
...
def score_candidate(url: str, label: str) -> tuple[int, str]:
    parsed = urlparse(url)
    haystack = fold(" ".join([parsed.path.replace("-", " "), parsed.path.replace("/", " "), label]))
    if any(term in haystack for term in NOISE_TERMS):
        return 0, "noise"
    matches = [(term, points) for term, points in LOCATION_TERMS if term in haystack]
    if not matches:
        return 0, "no-location-term"
...
def discover_location_links(
    base_url: str,
    html_text: str,
    max_links: int = MAX_LINKS_PER_CLINIC,
) -> list[LinkCandidate]:
    parser = LinkParser()
...
def source_metadata(clinic: dict[str, Any], candidate: LinkCandidate) -> dict[str, Any]:
    return {
        ...
        "reason": "official_contact_or_location_page_discovered_from_homepage",
...
def discovery_row(clinic: dict[str, Any], candidate: LinkCandidate, already_stored: bool) -> dict[str, Any]:
    return {
        ...
        **asdict(candidate),
        ...
        "source_type": LOCATION_SOURCE_TYPE,
...
def discover_for_clinic(
    ...
    try:
        ...
        links = discover_location_links(result.final_url or base, html_text, max_links=max_links_per_clinic)
...
def insert_sources_sql(rows: list[dict[str, Any]]) -> str:
    ...
    values = []
    for row in rows:
        title = f"{row.get('clinic_name') or row.get('clinic_slug')} · página de sedes/contacto"
...
inserted as (
  ...
  select
    ...
    {sql_literal(LOCATION_SOURCE_TYPE)},
    now(),
    incoming.metadata
  from incoming
  where not exists (
...
def summarize(results: list[dict[str, Any]], apply: bool, inserted_count: int = 0) -> dict[str, Any]:
    ...
    return {
        ...
        "location_sources_found": len(items),
        "new_location_sources": len(new_items),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--clinic-slug", help="Discover contact/location sources for one clinic.")
