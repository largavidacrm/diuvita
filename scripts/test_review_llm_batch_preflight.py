...
def main():
    ...
    check(blocked["strict_prompt_status"] == "blocked", "legacy reviews-only packets should not pass preflight")
    check(blocked["llm_readiness_status"] == "blocked_source_without_context", "legacy reviews-only readiness status missing")
    check(blocked["llm_ready"] is False, "legacy reviews-only packet should not be LLM-ready")
    check(blocked["next_step"] == "human_review_or_source_handoff", "legacy reviews-only next step missing")
    ...
    check("Preparables para LLM estricto: 3/4" in compact, "compact ready count missing")
