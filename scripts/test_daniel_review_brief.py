...
def main():
    ...
    check(
        first_step(portal_digest)[0] == "Primero revisa reclamaciones de ficha.",
        "portal-originated claim should be a ficha claim",
    )
    check("Caso visible: Solicitud de acceso: Monarka Clinic." in portal_output, "portal visible case missing")
    check("1 reclamación de ficha pendiente" in portal_output, "portal claim count missing")
    check("1 cambio pedido por clínica pendiente" in portal_output, "portal change count missing")
    check(
        "Estado: 2 pendientes: 1 reclamación de ficha, 1 cambio; 1 ficha con datos confirmados por el centro" in portal_output,
        "portal status summary missing",
    )
    check("Siguiente portal: Revisar 1 reclamación de ficha" in portal_output, "portal next action missing")
